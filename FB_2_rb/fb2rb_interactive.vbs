...
usageText="Usage: fb2rb_interactive.vbs [infile]"
FN=""
if args.Count>0 then
	...
	if args.Count>1 then
	  WScript.Echo "Only one parameter supported in interactive mode. Non-GUI script fb2rb_commandline.vbs supports all settings via parameters"
	  WScript.Quit 0
	end if
  FN=args(0)
end if
...
If Err Then
  WScript.Echo "Unable to create ActiveX object ""FB2_to_RB.FB2RBExport"". This may be because of the incorrect setup."&_
		vbCr & vbCr & "Please reinstall FB2Any and try rinning this script again."
  WScript.Quit 1
end if
