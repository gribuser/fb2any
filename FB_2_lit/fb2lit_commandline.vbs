...
if args.Count=0 then
  WScript.Echo "Usage: fb2lit_commandline.vbs <infile> [outfile] [options]" & vbCrLf & vbCrLf &_
...
Set FBApp = CreateObject("FB2_to_LIT.FB2LITExport")
If Err Then
  WScript.Echo "Unable to create ActiveX object ""FB2_to_LIT.FB2LITExportt"". This may be because of the incorrect setup."&_
...
if args.Count=1 or Left(args(1),1)="-" then
  regEx.Pattern = "\.[^\.]+$"
  filefolder = regEx.Replace(args(0),"") & ".lit"
else
  filefolder = args(1)
end if

xslpath=installFolder & "\FB2_2_lit_xhtml.xsl"
xslOEBpath=installFolder & "\FB2_2_lit_opf.xsl"
...
set opfXSL = WScript.CreateObject("Msxml2.FreeThreadedDOMDocument.4.0")
opfXSL.load(xslOEBpath)
Set FBApp.OPFXSL = opfXSL
