...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fb="http://www.gribuser.ru/xml/fictionbook/2.0" xmlns:xlink="http://www.w3.org/1999/xlink">
	...
	<xsl:template match="fb:body" mode="toc">
		<xsl:choose>
			<xsl:when test="@name = 'notes'"><br/><li><a href="#TOC_xsl_notes"><xsl:value-of select="$NotesTitle"/></a></li></xsl:when>
	...
	<!-- p -->
	<xsl:template match="fb:p"><xsl:call-template name="preexisting_id"/>&#160;&#160;&#160;<xsl:apply-templates/><br/></xsl:template>
		...
		<xsl:for-each select="fb:body">
			...
			<xsl:choose>
				<xsl:when test="@name = 'notes'">
						<h1><xsl:value-of select="$NotesTitle"/></h1>
<!--					<xsl:if test="not(.//fb:section[fb:title]/fb:section)">
					</xsl:if>-->
					<a name="TOC_xsl_notes"/>
				</xsl:when>
